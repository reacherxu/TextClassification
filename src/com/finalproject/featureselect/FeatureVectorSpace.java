...
public class FeatureVectorSpace {
	...
	public FeatureVectorSpace(Vector<String>stringVector,FeatureManager featureManager){
		this.featureVector=stringVector;
		fileCountOfFeature=new int[featureVector.size()];
		for(int i=0;i<featureVector.size();i++){
			fileCountOfFeature[i]=featureManager.getTotalFileCountOfFeature(featureVector.get(i));
		}
	}
